import { o as De, t as ye } from '../chunks/scheduler.e108d1fd.js';
import {
	S as He,
	a as Je,
	I as V,
	...
	P as qe,
	h as We
} from '../chunks/singletons.2786b6d8.js';
function Xe(n, o) {
	return n === '/' || o === 'ignore'
		? n
		: o === 'never'
...
function it(n, o) {
	...
	if (s != null && s.textContent) {
		const { body: d, ...f } = JSON.parse(s.textContent),
			S = s.getAttribute('data-ttl');
		return (
							...
							.map((s) => {
								...
								return (
									...
									S.map((y, w) => {
										if (w % 2) {
											if (y.startsWith('x+'))
												return be(String.fromCharCode(parseInt(y.slice(2), 16)));
											if (y.startsWith('u+'))
												return be(
													String.fromCharCode(
														...y
...
function pt({ nodes: n, server_loads: o, dictionary: u, matchers: s }) {
	const d = new Set(o);
...
async function St(n) {
	var o;
	for (const u in n)
		if (typeof ((o = n[u]) == null ? void 0 : o.then) == 'function')
			return Object.fromEntries(
				await Promise.all(Object.entries(n).map(async ([s, d]) => [s, await d]))
			);
	return n;
}
class te {
...
function Rt(n, o) {
	...
	async function oe(...e) {
		const t = u
			...
			.map((r) => Promise.all([...r.layouts, r.leaf].map((a) => (a == null ? void 0 : a[1]()))));
		await Promise.all(t);
	}
	function Oe(e) {
		var r;
		...
		const t = {
			...
			to: {
				params: h.params,
				route: { id: ((r = h.route) == null ? void 0 : r.id) ?? null },
	...
	async function W({ url: e, params: i, branch: t, status: r, error: a, route: c, form: p }) {
		let v = 'never';
		for (const g of t) (g == null ? void 0 : g.slash) !== void 0 && (v = g.slash);
		(e.pathname = Xe(e.pathname, v)), (e.search = e.search);
		const b = {
			type: 'loaded',
			state: { url: e, params: i, branch: t, error: a, route: c },
			props: { constructors: vt(t).map((g) => g.node.component) }
		};
		...
		for (let g = 0; g < Math.max(t.length, h.branch.length); g += 1) {
			const m = t[g],
				P = h.branch[g];
			(m == null ? void 0 : m.data) !== (P == null ? void 0 : P.data) && (R = !0),
				m && ((_ = { ..._, ...m.data }), R && (b.props[`data_${A}`] = _), (A += 1));
		}
		return (
			(!h.url || e.href !== h.url.href || h.error !== a || (p !== void 0 && p !== F.form) || R) &&
				(b.props.page = {
					error: a,
					params: i,
	...
	async function de({ loader: e, parent: i, url: t, params: r, route: a, server_data_node: c }) {
		var _, R, A;
		let p = null;
		...
		if ((_ = b.universal) != null && _.load) {
			...
			const g = {
				...
				params: new Proxy(r, { get: (m, P) => (v.params.add(P), m[P]) }),
				...
				async fetch(m, P) {
					...
					return (
						I(C.href),
						C.origin === t.origin && ($ = C.href.slice(t.origin.length)),
						x ? st($, C.href, P) : it($, P)
					);
				},
				setHeaders: () => {},
		...
		return {
			...
			loader: e,
			server: c,
			universal: (R = b.universal) != null && R.load ? { type: 'data', data: p, uses: v } : null,
			data: p ?? (c == null ? void 0 : c.data) ?? null,
			slash:
				((A = b.universal) == null ? void 0 : A.trailingSlash) ?? (c == null ? void 0 : c.slash)
		};
	}
	function Ue(e, i, t, r, a) {
		...
		for (const c of r.dependencies) if (S.some((p) => p(new URL(c)))) return !0;
		return !1;
	}
	function pe(e, i) {
	...
	async function he({ id: e, invalidating: i, url: t, params: r, route: a }) {
		...
		const g = b.map((E, O) => {
			...
			const L = h.branch[O],
				T =
					!!(E != null && E[0]) &&
					((L == null ? void 0 : L.loader) !== E[1] ||
						Ue(I, A, R, (J = L.server) == null ? void 0 : J.uses, r));
			return T && (I = !0), T;
		});
		if (g.some(Boolean)) {
			try {
				...
				return ie({
					...
					error: await Z(E, { url: t, params: r, route: { id: a.id } }),
					url: t,
					route: a
				});
			}
		...
		const $ = b.map(async (E, O) => {
			...
			if (
				(!T || T.type === 'skip') &&
				E[1] === (L == null ? void 0 : L.loader) &&
				!Ue(P, A, R, (ge = L.universal) == null ? void 0 : ge.uses, r)
			)
				return L;
		...
		for (let E = 0; E < b.length; E += 1)
			if (b[E])
				try {
					...
					if (O instanceof Me) return { type: 'redirect', location: O.location };
					let L = 500,
						T;
					if (m != null && m.includes(O)) (L = O.status ?? L), (T = O.error);
						...
						T = await Z(O, { params: r, url: t, route: { id: a.id } });
					}
	...
	async function ie({ status: e, error: i, url: t, route: r }) {
		...
		return await W({ url: t, params: a, branch: [v, b], status: e, error: i, route: null });
	}
	function X(e, i) {
		if (_e(e, K)) return;
		const t = se(e);
		for (const r of u) {
			const a = r.exec(t);
			if (a) return { id: e.pathname + e.search, invalidating: i, route: r, params: Qe(a), url: e };
		}
	}
	function se(e) {
		return Ze(e.pathname.slice(K.length) || '/');
	}
	function xe({ url: e, type: i, intent: t, delta: r }) {
		var v, b;
		let a = !1;
		const c = {
			from: {
				params: h.params,
				route: { id: ((v = h.route) == null ? void 0 : v.id) ?? null },
				url: h.url
			},
			to: {
				params: (t == null ? void 0 : t.params) ?? null,
				route: { id: ((b = t == null ? void 0 : t.route) == null ? void 0 : b.id) ?? null },
				url: e
			},
			willUnload: !t,
			type: i
	...
	async function ce({
		...
		var $, C, E;
		const R = X(e, !1),
			A = xe({ url: e, type: c, delta: p, intent: R });
		if (!A) {
			_();
			return;
		}
		...
		if (!g) {
			...
			g = await Ne(
				e,
				{ id: null },
				await Z(new Error(`Not found: ${e.pathname}`), { url: e, params: {}, route: { id: null } }),
				404
			);
		}
		...
		if (g.type === 'redirect')
			if (r.length > 10 || r.includes(e.pathname))
				g = await ie({
					status: 500,
					error: await Z(new Error('Redirect loop'), { url: e, params: {}, route: { id: null } }),
					url: e,
					route: { id: null }
				});
			else return re(new URL(g.location, e).href, {}, [...r, e.pathname], v), !1;
		else
			(($ = g.props.page) == null ? void 0 : $.status) >= 400 &&
		...
		if (
			...
			(C = g.props.page) != null &&
				...
				(e.pathname = (E = g.props.page) == null ? void 0 : E.url.pathname),
			a)
		) {
		...
		(y = null),
			x ? ((h = g.state), g.props.page && (g.props.page.url = e), q.$set(g.props)) : Oe(g);
		...
		!t && !P && Ee(),
			(k = !0),
			g.props.page && (F = g.props.page),
			(U = !1),
			c === 'popstate' && Ae(j),
			w.after_navigate.forEach((O) => O(A)),
	...
	function Ye() {
		...
		f.addEventListener('mousedown', i), f.addEventListener('touchstart', i, { passive: !0 });
		const t = new IntersectionObserver(
			(c) => {
				for (const p of c)
					p.isIntersecting && (oe(se(new URL(p.target.href))), t.unobserve(p.target));
			},
			{ threshold: 0 }
		);
		function r(c, p) {
			const v = Ve(c, f);
			if (!v) return;
			const { url: b, external: _, download: R } = we(v, K);
			if (_ || R) return;
			const A = le(v);
			if (!A.reload)
				if (p <= A.preload_data) {
		...
		function a() {
			...
			for (const c of f.querySelectorAll('a')) {
				const { url: p, external: v, download: b } = we(c, K);
				if (v || b) continue;
				const _ = le(c);
				_.reload ||
					(_.preload_code === qe.viewport && t.observe(c),
					_.preload_code === qe.eager && oe(se(p)));
			}
		}
		...
		_start_router: () => {
			...
			(history.scrollRestoration = 'manual'),
				addEventListener('beforeunload', (t) => {
					...
					if ((Ie(), !U)) {
						const c = {
							from: {
								params: h.params,
								route: { id: ((a = h.route) == null ? void 0 : a.id) ?? null },
								url: h.url
							},
							to: null,
							willUnload: !0,
				...
				f.addEventListener('click', (t) => {
					...
					if (!r) return;
					const { url: a, external: c, target: p, download: v } = we(r, K);
					if (!a) return;
					if (p === '_parent' || p === '_top') {
					...
					if (
						...
						return;
					if (c || b.reload) {
						xe({ url: a, type: 'link' }) ? (U = !0) : t.preventDefault();
						return;
					}
					...
					if (A !== void 0 && R === location.href.split('#')[0]) {
						if (h.url.hash === a.hash) {
							t.preventDefault(),
								(I = r.ownerDocument.getElementById(A)) == null || I.scrollIntoView();
							return;
						}
				...
				f.addEventListener('submit', (t) => {
					...
					if (_e(p, K)) return;
					const v = t.target,
						{ keep_focus: b, noscroll: _, reload: R, replace_state: A } = le(v);
					if (R) return;
					t.preventDefault(), t.stopPropagation();
					const I = new FormData(v),
						g = a == null ? void 0 : a.getAttribute('name');
					g && I.append(g, (a == null ? void 0 : a.getAttribute('value')) ?? ''),
						(p.search = new URLSearchParams(I).toString()),
						ce({
							url: p,
							scroll: _ ? ee() : null,
							keepfocus: b ?? !1,
				...
				addEventListener('hashchange', () => {
					B && ((B = !1), history.replaceState({ ...history.state, [V]: ++j }, '', location.href));
				});
			...
			function e(t) {
				(h.url = t), M.page.set({ ...F, url: t }), M.page.notify();
			}
		},
		_hydrate: async ({
			status: e = 200,
			...
			({ params: r = {}, route: a = { id: null } } = X(v, !1) || {});
			let b;
			try {
				const _ = t.map(async (I, g) => {
						const m = c[g];
						return (
							m != null && m.uses && (m.uses = Ge(m.uses)),
							de({
								loader: n.nodes[I],
								url: v,
								params: r,
				...
				b = await ie({
					...
					error: await Z(_, { url: v, params: r, route: a }),
					url: v,
					route: a
				});
			}
					...
					(h = N.nodes) == null ||
						h.forEach((U) => {
							(U == null ? void 0 : U.type) === 'data' &&
								((U.uses = Ge(U.uses)), (U.data = y(U.data)));
						}),
						d(N);
				else if (N.type === 'chunk') {
					const { id: U, data: B, error: H } = N,
						q = f.get(U);
...
async function Lt(n, o, u) {
	const s = Rt(n, o);
	We({ client: s }),
		u ? await s._hydrate(u) : s.goto(location.href, { replaceState: !0 }),
		s._start_router();
}
export { Lt as start };
